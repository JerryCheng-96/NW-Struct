...
class CPyAmino(Structure):
    _fields_ = [("num", c_int)]


# Python helper functions

def PDB2CAminoPtrArray(seq_len):
    pAminoArray = (POINTER(CPyAmino) * seq_len)()
    aminos_seq = ''

    for i in range(0, seq_len):
        pAminoArray[i] = pointer(CPyAmino(i))
        aminos_seq += 'A'

    pAminoArray = cast(pAminoArray, POINTER(POINTER(CPyAmino)))
